...
- **登录态与黑盒（v9.4）**：不得从截图读取凭据、代替真人完成验证码/短信/二次核身。**没有测试账号 ≠ 停手**：登录墙是黑盒测试的起点，不是终点。遇到登录墙必须先执行「无凭据黑盒协议」（§7 认证面分支），按顺序穷尽：① 注册开放则自助注册 ≥2 个同级账号恢复带态测试；② 未认证攻击面全覆盖；③ 认证机制本身作为攻击面；④ 全部穷尽后才输出 `NEED_INPUT`，且必须附已尝试步骤、落盘证据和需要人工提供的具体内容。「所有业务接口都在登录后」不是 NEED_INPUT 的充分条件。
...
- **按认证面分支（一等分支）**
  ...
  - **无凭据黑盒协议（登录墙 ≠ 停手）**：没有测试账号时按固定顺序推进，每一步证据落盘后才进入下一步：
    1. **自助注册优先**：注册接口开放（含邀请码可获取/可绕过）时，自行注册 ≥2 个同级账号作为 owner/attacker，立即恢复全量带态测试；注册流程中的 captcha/SMS/邮箱验证/邀请码按上条「二次验证绕过优先协议」处理，绕过成功本身就是 finding。
    2. **未认证面全覆盖**：登录墙前的 surface 是黑盒主战场——公开页面、JS 提取的端点、匿名可达 API、错误信息/指纹泄露照常进入覆盖台账并闭合；匿名 200 只记可达，不直接报未授权（公开性基线铁律）。
    3. **认证机制攻击**：登录/注册/找回/重置流程本身即高价值面——用户枚举三通道、验证码各绕过方向、token 可预测性与重放、流程跳步、批量注册/弱口令策略（不得压测式爆破）。
    4. **穷尽判定**：以下三条**同时**满足才允许 `NEED_INPUT`——① 注册关闭且绕过协议 ≥5 方向失败；② 未认证面已充分覆盖且无进展信号；③ 认证机制攻击无进展。输出时必须写明已尝试步骤、证据路径、需要人工提供什么（账号 / 邀请码 / 手机号 / 内网入口等）。只满足部分 → 继续测或将带态格标 `blocked + next_actions`，终态只能 `incomplete`，不得 LOW_ROI。
- **按参数语义分支**
