...
1. 页面与 JS 分析：提取所有 JS 文件并 grep API 路径 → `endpoint_inventory.md`；v10.0 起可先用采集器自动抓取页面/JS 快照产出现料：`python3 -m engine.skill_runtime recon --target <url> --scope-file AUTHZ.md --out testdata/runs/<sid>/recon`，再以 `init --recon-dir testdata/runs/<sid>/recon` 消费
