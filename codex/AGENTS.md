...
- **逐格阴性文件格式**：每条阴性结论 = 一篇独立 `negative_<surface_id>.md`，frontmatter 必含 `endpoint / method / param / vuln / roles / identities / vectors / evidence_types / next_actions`，正文附关键请求/响应片段。证据不足时只能标 `shallow_negative` 并留 `next_actions`，**严禁** 写 `not_vulnerable`。
- **阴性抬阈值（knowledge cards，可抬不可降）**：注入类 ≥ 3 个 payload 家族；SSRF ≥ `{0.0.0.0, 127.0.0.1, DNS-rebinding, @-前缀, 多协议(file://, gopher://)}`；IDOR/越权 ≥ 2 角色 × 2 对象 + 响应差异。未达阈值一律 `shallow_negative`，不得 `not_vulnerable`。
- **禁止单篇合并散文阴性**：不得把多条阴性塞进一篇 `negative_findings.md` 自由散文——逐格 ingest 只认 `negative_<surface_id>.md` 的 frontmatter，合并散文会被吃下却贡献为空，等于没落盘。
...
- **风险维逐维应答（v6.1 硬纪律）**：每个攻击面按其 risk_tags 展开成风险维清单后，必须对**每一维**应答——有假设就写候选（CANDIDATE），没有就写 NONE:<reason>，**不许跳过任何一维**。这是"榨干"的确定性骨架：不是靠你自觉发散，而是逐维强制应答。换皮重述同一候选（同 endpoint+漏洞类+参数+角色+对象+根因）不算新增，会被 diversity_key 拦。
- **depth floor（取证完整性，非"进一步利用"）**：确认漏洞前，候选必须达到 depth_floor——即用多向量/多角色/多对象的对比证据证明这**真是漏洞且严重度没被低估**。IDOR/越权 ≥2 角色×2 对象；业务逻辑篡改 ≥3 金额向量；注入 ≥3 探测向量家族。**这是取证完整性，不是"进一步利用"**——前者必做（§4 铁律"够证明即止"不豁免 depth floor），后者红线禁止。depth_score < depth_floor 时候选不得标 proof_ready。
...
`finding.json` 必须包含：`schema_version/id/title/severity/vuln_type/target/risk/recommendation/feature_point 或 source_proof/apis/proof_packets/manual_burp_replay/poc`。其中：

- `risk.proven_impact` 只能写已证明结果，不能写「可能/疑似/理论上」。
- `proof_packets[].request_file`、`proof_packets[].response_file`、`poc.file` 对应文件必须真实存在。
- 从 JS/源码/配置构造数据包时，`source_proof` 必填，并写明文件、行号或函数、构造出的请求包。
- 有加密链路时，`crypto_chain` 必填，并写明算法、key/iv 来源、解密/重加密方式和 helper 文件。
- Burp 手工复现步骤至少 2 步，能让人工按包复测。
