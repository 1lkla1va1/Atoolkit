...
0. 先定位当前 **Atoolkit** 项目根，读取项目根 `AGENTS.md` 和 `SKILL.md`；不得假设全局 `~/.codex/AGENTS.md` 或其他项目的 `/src` 已注入。运行 `python3 run.py --doctor --workspace-root <当前工作区>`，工作区指令哈希或 `/src` 归属不一致时终止。
0.1 Engine 使用 `run.py`；Wrapped Skill 由 agent 外部的 `python3 -m engine.skill_wrapper` 建立 manifest/run plan；没有 wrapper 的 Direct/Qoder 在首个 recon 前运行 `python3 -m engine.skill_runtime preflight --workspace-root <当前工作区>`，Phase 0 后运行 `init`，每批实验后运行 `checkpoint`。初始化/checkpoint 失败、proof repair 非零或出现 finalizer 保留产物时不得继续写终态。
0.2 只有外部容器/cgroup/job 监督器能证明所有后代已静默时才可信。当前内置 backend、wrapper 和 Direct Skill 都是 diagnostic，不得声称 `authority_trusted=true` 或完整交付。
