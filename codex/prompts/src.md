...
0.1 在首个网络动作前，用 `python3 -m engine.runtime_manifest init-manifest --mode skill --run-dir <session> --primary-target <url> --allow <scope> --authz-file <authz.md> --instruction AGENTS.md --instruction SKILL.md` 生成运行指纹；manifest 失败时不得继续。
...
6. 任一方向 **20 分钟无进展 → 换攻击面**；结束前必须运行 `python3 -m engine.reporting.cli <session>`（空结果仅在完整闭环时显式 `--allow-empty`），再运行 `python3 -m engine.runtime_manifest receipt --run-dir <session>`，最后一行只输出一个终态标记。
