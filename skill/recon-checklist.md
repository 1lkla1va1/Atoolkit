...
注意：302 通常是跳转到登录页，403 是权限不足（端点存在但权限不够）。匿名请求返回数据只说明“可公开访问”，不是未授权漏洞结论。必须继续检查公开首页/列表/API 文档是否正常暴露相同内容，并取得 owner-only、authenticated-only 或 role-restricted 的物理证据；否则归类为公开端点。
